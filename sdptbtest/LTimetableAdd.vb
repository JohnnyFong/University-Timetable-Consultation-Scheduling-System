Public Class LTimetableAdd
    ...
    Function enroldetail(ByVal mdl As String, ByVal intake As String)
        Dim sql As String = "SELECT Lecture.LectureID, Lecture.LectureName, Course.Duration_pc FROM (Intake_Module LEFT JOIN Lecture ON Intake_Module.LectureID = Lecture.LectureID) LEFT JOIN Course ON Intake_Module.ModuleID = Course.ModuleID WHERE Intake_Module.ModuleID=@mdl AND IntakeID = @int"
        Dim dr As OleDb.OleDbDataReader
        Dim cmd As New OleDb.OleDbCommand(sql, conn)
        cmd.Parameters.AddWithValue("@mdl", mdl)
        ...
        Else
            dr.Read()
            lblselectedlec.Text = dr.Item(0) + " " + dr.Item(1)
            If dr.Item(1).Equals("1") Then
                lblduration.Text = dr.Item(2) + " hour"
            Else
                lblduration.Text = dr.Item(2) + " hours"
            End If
    ...
    Private Sub btnconf2_Click(sender As Object, e As EventArgs) Handles btnconf2.Click
        ...
        Dim lectureid As String() = lblselectedlec.Text.Split(" ")
        Dim lecsql As String = "SELECT * FROM (Timetable LEFT JOIN Course ON Timetable.ModuleID = Course.ModuleID) LEFT JOIN Intake_Module ON (Intake_Module.ModuleID = Timetable.ModuleID AND Intake_Module.IntakeID = Timetable.IntakeID) WHERE NOT ( Tdate >= DateAdd( 'n',(@duration*60),CDate(@date)) OR DateAdd( 'n',(Course.Duration_pc*60),Tdate) <=CDate(@date) ) AND Intake_Module.LectureID = @lect"
        Dim lecdr As OleDb.OleDbDataReader
        Dim leccmd As New OleDb.OleDbCommand(lecsql, conn)
        leccmd.Parameters.AddWithValue("@duration", Integer.Parse(lblduration.Text.Substring(0, 1)))
        leccmd.Parameters.AddWithValue("@date", datetostring(selecteddate))
        leccmd.Parameters.AddWithValue("@lect", lectureid(0))
        conn.Open()
        lecdr = leccmd.ExecuteReader()
        If lecdr.HasRows() Then
            MessageBox.Show("The lecture is teaching on the selected time")
            conn.Close()
            Exit Sub
        End If
        conn.Close()
